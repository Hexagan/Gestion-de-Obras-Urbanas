...
db = SqliteDatabase("db/obras_urbanas.db", pragmas={"journal_mode": "wal"})
...
class Obra(BaseModel):
    ...
    def iniciar_contratacion(self, contratacion, nro_contratacion):
        ...
        try:
            # Buscar y guardar ID
            contratacion_id = Contratacion.get(
                Contratacion.contratacion_tipo == contratacion.strip().upper()
            ).id
    ...
    def adjudicar_obra(self, empresa, expediente_numero):
        ...
        try:
            # Buscar y guardar ID
            empresa_id = Empresa.get(
                Empresa.licitacion_oferta_empresa == empresa.strip().upper()
            ).id
    ...
    def iniciar_obra(
        self, destacada, fecha_inicio, fecha_fin_inicial, financiamiento_tipo, mano_obra
    ):
        ...
        try:
            # Buscar y guardar ID
            financiamiento_id = Financiamiento.get(
                Financiamiento.financiamiento == financiamiento_tipo.strip().upper()
            ).id

...
# Definir superclase y métodos de forma abstracta
...
class GestionarObraCSV(GestionarObra):
    ...
    def limpiar_datos(self, datos: pd.DataFrame) -> pd.DataFrame:
        try:
            ...
            datos["destacada"] = datos["destacada"].apply(
                lambda x: True if x == "SI" else False
            )
            ...
            datos["monto_contrato"] = datos["monto_contrato"].apply(
                lambda x: (
            ...
            datos["plazo_meses"] = datos["plazo_meses"].apply(
            ...
            self.barrios_comunas.loc[:, "comuna"] = pd.to_numeric(
                self.barrios_comunas["comuna"], errors="coerce"
            )
            self.barrios_comunas = self.barrios_comunas[
                self.barrios_comunas["comuna"].notna()
            ]
        ...
        try:
            ...
            for _, row in datos.iterrows():
                ...
                Obra.create(
                    ...
                    barrio=Barrio.get_or_create(barrio=row["barrio"].strip().upper())[
                        0
                    ],
                    comuna=row["comuna"],
                    monto_contrato=row["monto_contrato"],
                    etapa=Etapa.get_or_create(etapa=row["etapa"].strip().upper())[0],
                    contratacion_tipo=Contratacion.get_or_create(
                        contratacion_tipo=row["contratacion_tipo"].strip().upper()
                    )[0],
                    nro_contratacion=row["nro_contratacion"],
                    licitacion_oferta_empresa=Empresa.get_or_create(
                    ...
                    financiamiento=Financiamiento.get_or_create(
                        financiamiento=row["financiamiento"].strip().upper()
                    )[0],
    ...
    def obtener_indicadores(self) -> dict:
        try:
            ...
            indicadores = {
                ...
                "obras_finalizadas_y_monto_inversión_comuna_1": {},
            }
            ...
            try:
                etapas = (
                    Obra.select(
                        Etapa.etapa.alias("etapa_nombre"),
                        fn.COUNT(Obra.id).alias("etapa_cantidad"),
                    )
            ...
            try:
                comunas = self.barrios_comunas[
                    self.barrios_comunas["comuna"].isin([1, 2, 3])
                ]
            ...
            try:
                obras_comuna_1 = Obra.select().where(
                    (Obra.comuna == 1) & (Obra.etapa == 3)
                )
                cantidad_obras_comuna_1 = obras_comuna_1.count()
                total_inversion_obras_comuna_1 = (
                    Obra.select(fn.SUM(Obra.monto_contrato))
                    .where((Obra.comuna == 1) & (Obra.etapa == 3))
                    .scalar()
                )
                indicadores["obras_finalizadas_y_monto_inversión_comuna_1"] = {
                    "cantidad": cantidad_obras_comuna_1,
                    "total_inersion": total_inversion_obras_comuna_1,
                }
            except OperationalError as error:
                print(f"Error al obtener obras finalizadas en comuna 1: {error}")

            # Cantidad de obras finalizadas en un plazo menor o igual a 24 meses
            try:
                obras_24_meses = (
                    Obra.select()
                    .where((Obra.etapa == 3) & (Obra.plazo_meses <= 24))
                    .count()
                )
                indicadores["obras_finalizadas_24_meses"] = obras_24_meses
            except OperationalError as error:
                print(
                    f"Error al obtener obras finalizadas en 24 meses o menos: {error}"
                )
            ...
            try:
                ...
                porcentaje_finalizadas = (
                    (obras_finalizadas / total_obras) * 100 if total_obras > 0 else 0
                )
            ...
            try:
                ...
                print(
                    f"Error al obtener cantidad total de mano de obra empleada: {error}"
                )
